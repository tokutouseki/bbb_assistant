...
TTS_OUTPUT_DIR = os.path.join(project_root, "outputs", "test_tts_asr")
...
def main():
    print("=" * 70)
    print("  Qwen3-TTS 爱莉希雅预设 → 生成音频 → ASR 识别 联动测试")
    print("=" * 70)

    # ===================== Step 1: TTS 生成 =====================
    print("\n[Step 1] 初始化 Qwen3-TTS 生成器...")
    ...
    print(f"  模型初始化: {'成功' if tts.initialized else '模拟模式'}")
    print(f"\n[Step 2] 使用「爱莉希雅」预设生成语音...")
    ...
    print(f"  音频长度: {len(tts_result.audio_data)} samples ({len(tts_result.audio_data)/tts_result.sample_rate:.2f}s)")
    ...
    model_ok = asr.model is not None
    print(f"  模型加载: {'成功' if model_ok else '模拟模式'}")
    ...
    print(f"""
  ┌─────────────────────────────────────────────────────────────┐
  │  TTS 引擎       : Qwen3-TTS (爱莉希雅预设)                   │
  │  TTS 采样率     : {tts_result.sample_rate} Hz                                        │
  │  TTS 耗时       : {tts_time:.2f}s                                           │
  │  TTS 音频文件   : {audio_path}
  │  ASR 引擎       : SenseVoiceSmall ({'真实模型' if model_ok else '模拟模式'})           │
