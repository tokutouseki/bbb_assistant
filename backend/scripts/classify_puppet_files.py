...
# -*- coding: utf-8 -*-
"""
脚本功能：收集人偶目录中所有JSON文件的URL，并通过面包屑导航进行分类
使用方法：直接运行此脚本
"""
...
URL_FILE = r"d:\TokusCode\bbb_assistant\backend\data\人偶文件URL列表.txt"
OUTPUT_FILE = r"d:\TokusCode\bbb_assistant\backend\data\人偶文件分类结果.txt"
...
async def main():
    print("开始收集人偶目录中的URL并进行分类...")
    print("=" * 80)
    
    json_files = [f for f in os.listdir(PUPPET_DIR) if f.endswith('.json')]
    ...
    for file_name in sorted(json_files):
        file_path = os.path.join(PUPPET_DIR, file_name)
        try:
