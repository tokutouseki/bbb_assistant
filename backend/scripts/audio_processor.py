...
"""
批量音频处理工具
对指定目录中的音频文件进行ASR转录和TTS合成，用于语音克隆测试和音频处理
支持崩坏3角色语音识别和音色模仿
"""
...
class BatchAudioProcessor:
    ...
    def __init__(self, 
        ...
        Args:
            input_dir: 输入音频目录
            output_dir: 输出目录（如果为None，则使用项目根目录下的outputs/audio_processing）
            model_device: 模型设备（cuda:0 或 cpu）
            use_simulated_mode: 是否使用模拟模式（不加载真实模型）
            voice_mapping_file: 语音映射配置文件路径（JSON格式）
        """
        ...
        if output_dir is None:
            self.output_dir = project_root / "outputs" / "audio_processing"
        else:
        ...
        # 加载语音映射
        self.voice_mapping = self._load_voice_mapping(voice_mapping_file)
        
        logger.info(f"批量音频处理器初始化完成")
        logger.info(f"输入目录: {self.input_dir}")
        logger.info(f"输出目录: {self.output_dir}")
        logger.info(f"设备: {self.device}")
        logger.info(f"模拟模式: {self.use_simulated_mode}")
        logger.info(f"语音映射数量: {len(self.voice_mapping)}")
    
    def _load_voice_mapping(self, mapping_file: Optional[str]) -> Dict[str, str]:
        """
        加载语音映射
        优先使用配置文件，如果没有则使用默认映射
        ...
        default_mapping = {
            ...
            "Da_Yue_Xia": "theresa",  # 大月下，与月下誓约是同一角色
        ...
        if mapping_file and Path(mapping_file).exists():
            try:
                ...
                # 合并映射，文件中的映射优先
                merged_mapping = {**default_mapping, **file_mapping}
                logger.info(f"从文件加载语音映射: {mapping_file}")
                return merged_mapping
            except Exception as e:
                logger.warning(f"加载语音映射文件失败: {e}，使用默认映射")
                return default_mapping
        else:
            if mapping_file:
                logger.warning(f"语音映射文件不存在: {mapping_file}，使用默认映射")
            return default_mapping
    ...
    def _initialize_simulated_processors(self):
        ...
        class SimulatedASRProcessor:
            ...
            def transcribe_file(self, audio_path, language="zh", save_result=False):
                ...
                if "德丽莎" in filename or "月下誓约" in filename or "Da_Yue_Xia" in filename:
                    text = "这是德丽莎（月下誓约）的模拟语音识别结果。"
                elif "朔夜观星" in filename:
                    text = "这是朔夜观星的模拟语音识别结果。"
                elif "渡尘之羽" in filename or "识之律者" in filename:
                    text = "这是符华的模拟语音识别结果。"
                elif "迷城骇兔" in filename:
                    text = "这是布洛妮娅（迷城骇兔）的模拟语音识别结果。"
    ...
    def process_audio_file(self, audio_file: Path) -> Optional[AudioProcessingResult]:
        """
        处理单个音频文件
    ...
    def _generate_report(self):
        ...
        try:
            ...
            # 保存JSON报告
            json_report_path = self.report_dir / f"processing_report_{int(time.time())}.json"
            with open(json_report_path, 'w', encoding='utf-8') as f:
                json.dump(report_data, f, ensure_ascii=False, indent=2)
            
            # 生成文本报告
            text_report_path = self.report_dir / f"processing_report_{int(time.time())}.txt"
            with open(text_report_path, 'w', encoding='utf-8') as f:
                f.write("=" * 60 + "\n")
                f.write("批量音频处理报告\n")
                f.write("=" * 60 + "\n\n")
                
                f.write(f"处理时间: {report_data['processing_info']['generated_at']}\n")
                f.write(f"输入目录: {report_data['processing_info']['input_dir']}\n")
                f.write(f"输出目录: {report_data['processing_info']['output_dir']}\n")
                f.write(f"设备: {report_data['processing_info']['device']}\n")
                f.write(f"模拟模式: {report_data['processing_info']['use_simulated_mode']}\n\n")
                
                f.write(f"文件总数: {report_data['processing_info']['total_files']}\n")
                f.write(f"成功文件: {report_data['processing_info']['successful_files']}\n")
                f.write(f"失败文件: {report_data['processing_info']['failed_files']}\n")
            ...
            print("\n" + "=" * 60)
            print("处理摘要:")
            print(f"  总文件数: {report_data['processing_info']['total_files']}")
            print(f"  成功文件: {report_data['processing_info']['successful_files']}")
            print(f"  失败文件: {report_data['processing_info']['failed_files']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="批量音频处理工具 - 支持ASR转录和TTS合成",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s --input-dir ./audio_files --device cpu
  %(prog)s --input-dir ./audio_files --output-dir ./results --simulated
  %(prog)s --input-dir ./audio_files --list-files
  %(prog)s --input-dir ./audio_files --voice-mapping mapping.json
        """
    )
    
    parser.add_argument("--input-dir", type=str, 
                       default=r"d:\TokusCode\bbb_assistant\to_clone_test",
                       help="输入音频目录（默认: d:\\TokusCode\\bbb_assistant\\to_clone_test）")
    parser.add_argument("--output-dir", type=str, default=None,
                       help="输出目录（默认: 项目根目录/outputs/audio_processing）")
    parser.add_argument("--device", type=str, default="cuda:0",
                       help="模型设备: cuda:0 或 cpu（默认: cuda:0）")
    parser.add_argument("--simulated", action="store_true",
                       help="使用模拟模式（不加载真实模型）")
    parser.add_argument("--voice-mapping", type=str, default=None,
                       help="语音映射配置文件路径（JSON格式）")
